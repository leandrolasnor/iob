...
RUN apk update && apk add \
    ...
    nano curl font-meslo-nerd shadow zsh-vcs dos2unix yarn

ENV APP_HOME /app
WORKDIR $APP_HOME

RUN git init
RUN git remote add origin https://github.com/leandrolasnor/iob
...
RUN sed -i 's/plugins=(git)/plugins=(git kubectl zsh-autosuggestions zsh-syntax-highlighting)/g' ~/.zshrc
